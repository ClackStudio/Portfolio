...
const Bar = ({
  ...
  const howManyBarsAfter = (numberOfBars, index) =>
    [...Array(numberOfBars).keys()].filter((number) => number >= index).length
  const howManyBarsBefore = (numberOfBars, index) =>
    [...Array(numberOfBars).keys()].filter((number) => number <= index).length
  ...
  useFrame((state, delta) => {
    if (typeof ref.current !== 'undefined' && !isMobile) {
      ...
      const scalePercentage =
        (numberOfBars - barsBetweenHover + 1) / numberOfBars
      ...
      const debug = () => {
        ...
        console.log(
          '((extraWidth * scalePercentage) ) ',
          extraWidth * scalePercentage
        )
      }

      const expandBar = () => {
        ref.current.material.scale = ref.current.scale.x = damp(
          ref.current.scale.x,
          imageWidth,
          lambda,
          delta
      ...
      const moveBarWithScale = (direction) => {
        ...
        if (isBeforeMiddle) {
          if (!hoveredBarOpenedToTheRight || index > currentHoveredBar) {
            ...
            ref.current.position.x = damp(
              ref.current.position.x,
              position[0] + (isFirst ? 0 : width) * scalePercentage * direction,
              lambda,
              delta
            )
          } else {
          ...
          ref.current.position.x = damp(
            ref.current.position.x,
            position[0] + extraWidth * scalePercentage * direction,
            lambda,
            delta
          )
        } else if (isAfterMiddle) {
          if (hoveredBarOpenedToTheRight || index < currentHoveredBar) {
            ...
            ref.current.position.x = damp(
              ref.current.position.x,
              position[0] +
                (isLast ? 0 : extraWidth) * scalePercentage * direction,
              lambda,
              delta
            )
          } else {
    ...
    >
      ...
      {currentHoveredBar === index ? (
        <meshBasicMaterial
          ...
          opacity={currentHoveredBar === index ? 1 : 0}
          map={map}
        />
      ) : (
